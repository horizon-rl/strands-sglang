...
class DeepSeekV32ToolParser(ToolParser):
    ...
    def __init__(self, *, model_name_or_path: str | None = None, **kwargs: Any) -> None:
        ...
        Args:
            model_name_or_path: HuggingFace model ID or local path
                (e.g. ``"deepseek-ai/DeepSeek-V3.2"``).  When provided, the
                model's ``encoding/encoding_dsv32.py`` is loaded and used
                directly in :meth:`format_prompt`.  When ``None``,
                ``format_prompt`` returns ``None`` (the caller should use
                ``tokenizer.apply_chat_template()`` instead).
        ...
        self._encoding_module: types.ModuleType | None = None
    ...
    def format_prompt(
        ...
        *,
        ...
        Returns ``None`` when no encoding module is loaded (no ``model_name_or_path``).
        """
        if self._encoding_module is None:
            return None
